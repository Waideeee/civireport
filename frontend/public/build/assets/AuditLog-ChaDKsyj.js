let d=1,u=[],o=[],p="",m="newest";function i(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function b(t){return t&&String(t).replace("T"," ").slice(0,16)||"—"}function g(t){if(!t)return'<span class="audit-badge badge-pending">Unknown</span>';let e=t,n="";if(t.includes(" (")){const a=t.split(" (");e=a[0],n=" ("+a[1]}const s={Pending:"badge-pending","In Progress":"badge-inprogress","in progress":"badge-inprogress",Approved:"badge-approved",Rejected:"badge-rejected",Resolved:"badge-approved"};let l=s[e];if(!l){const a=Object.keys(s).find(_=>_.toLowerCase()===e.toLowerCase());l=a?s[a]:"badge-pending"}let r=`<span class="audit-badge ${l}">${i(e)}</span>`;return n&&(r+=`<div style="font-size: 0.75rem; color: #6b7280; margin-top: 4px; font-weight: 500;">${i(n)}</div>`),r}function h(t){return t.complaint_id?`#C-${String(t.complaint_id).padStart(3,"0")}`:t.emergency_id?`#E-${String(t.emergency_id).padStart(3,"0")}`:"Resident Account"}function f(t){return[...t].sort((e,n)=>m==="newest"?n.audit_id-e.audit_id:e.audit_id-n.audit_id)}function v(){const t=document.getElementById("audit-log");t&&(t.innerHTML=`
  ...
  `,E())}function c(){const t=document.getElementById("al-tbody");if(!t)return;const e=f(o),n=Math.ceil(e.length/8)||1,s=(d-1)*8,l=e.slice(s,s+8),r=l.length?l.map(a=>`
        <tr>
